...
# Make /admin/database/seed accessible without authentication for initial setup
@app.get("/admin/database/seed")
async def seed_database_get():
    try:
        # Initialize the database with seed data
        if init_db():
            return {
                "status": "success",
                "message": "Database has been seeded successfully. You can now login with username: admin, password: admin123"
            }
        else:
            raise HTTPException(
                status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
